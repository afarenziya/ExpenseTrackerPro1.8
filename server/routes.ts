...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/reports/pdf", requireAuth, requirePermission("export_reports"), async (req, res) => {
    ...
    pdfDoc.setAuthor("ExpenseTracker Made By Ajay Farenziya");
    ...
    page.drawText(`Total Expenses: Rs.${totalAmount.toFixed(2)}`, {
      x: 50,
      y: height - 100,
      size: 12,
      font: boldFont,
    ...
    for (const expense of expenses) {
      ...
      page.drawText(`Rs.${expense.amount.toFixed(2)}`, { x: startX + columnWidths[0] + columnWidths[1] + columnWidths[2] + columnWidths[3] + columnWidths[4], y: currentY, size: 9, font });
      
      currentY -= 20;
    }
    
    // Footer
    page.drawText("Generated by ExpenseTracker", {
      x: 50,
      y: 30,
      size: 8,
      font,
